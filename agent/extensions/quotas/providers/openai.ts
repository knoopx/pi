...
const loadOpenAIToken = (deps: BaseDependencies) => loadTokenFromPiAuthJson(deps, "openai-codex");
  ...
  windows: [
    {
      ...
      label: (data, windowData) => {
        const hours = Math.round((windowData.limit_window_seconds || 10800) / 3600);
        return `${hours}h`;
      },
    ...
    },
    {
      ...
      label: (data, windowData) => {
        const hours = Math.round((windowData.limit_window_seconds || 86400) / 3600);
