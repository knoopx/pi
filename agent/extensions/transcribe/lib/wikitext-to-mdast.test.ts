...
function c(n: unknown): MdastContent {
  return n as unknown as MdastContent;
}

function expectHeading(
  tree: Awaited<ReturnType<typeof wikitextToMdast>>,
  index = 0,
): MdastContent {
  const node = c(tree.children[index] as unknown);
...
function expectParagraph(
  ...
  const para = c(tree.children[index] as unknown);
  expect(para.type).toBe("paragraph");
  return para;
}

function countType(para: MdastContent, type: string): number {
  return (para.children ?? []).filter((x: unknown) => c(x).type === type)
    .length;
}

function expectTextCount(para: MdastContent, expected: number): void {
  const texts = (para.children ?? []).filter(
    (x: unknown) => c(x).type === "text",
  );
  expect(texts.length).toBe(expected);
}

describe("wiktextToMdast", () => {
  describe("headings", () => {
    it("parses level-2 heading with spaces", () => {
      const tree = wikitextToMdast("== Heading ==");
      expect(tree.children).toHaveLength(1);
      expectHeading(tree); // fallow-ignore-next-line complexity
      expect((c(tree.children[0] as unknown) as { depth?: number }).depth).toBe(
        2,
      );
    });
  ...
  describe("bold and italic", () => {
    it("parses bold text with triple apostrophes", () => {
      ...
      expect(tree.children).toHaveLength(1);
      const para = expectParagraph(tree);
      expect(c(para.children?.[0] as unknown)).toMatchObject({
        type: "strong",
      });
    });
    ...
    it("parses bold and italic combined", () => {
      ...
      expect(tree.children).toHaveLength(1);
      const para = expectParagraph(tree);
      expect(c(para.children?.[0] as unknown)?.type).toBe("strong");
    });
    ...
    it("handles multiple bold sections in one paragraph", () => {
      ...
      expect(tree.children).toHaveLength(1);
      const para = expectParagraph(tree);
      expect(countType(para, "strong")).toBe(2);
    });
  ...
  describe("wiki links", () => {
    it("parses simple wiki link", () => {
      ...
      expect(tree.children).toHaveLength(1);
      const para = expectParagraph(tree);
      const link = c(para.children?.[0] as unknown);
    ...
    it("parses wiki link with display text", () => {
      ...
      expect(link?.type).toBe("link");
      expect(link?.url).toBe("page");
      expect(c(link?.children?.[0])?.value).toBe("display");
    });
    ...
    it("parses wiki link with spaces in page name", () => {
      ...
      expect(tree.children).toHaveLength(1);
      const para = expectParagraph(tree);
      expect(c(para.children?.[0] as unknown)?.url).toBe("my_page");
    });

    it("handles multiple links in text", () => {
      const tree = wikitextToMdast("See [[page1]] and [[page2|link]]");
      const para = expectParagraph(tree);
      expect(countType(para, "link")).toBe(2);
    });
    ...
    it("handles link with display containing special chars", () => {
      ...
      const para = expectParagraph(tree);
      const link = c(para.children?.[0] as unknown);
      expect(c(link?.children?.[0])?.value).toBe("display (with parens)");
    });
  ...
  describe("thematic breaks", () => {
    it("parses thematic break with four dashes", () => {
      ...
      expect(tree.children).toHaveLength(1);
      expect(c(tree.children[0] as unknown).type).toBe("thematicBreak");
    });

    it("parses three dashes as thematic break (markdown standard)", () => {
      const tree = wikitextToMdast("---");
      const breaks = tree.children.filter(
        (x: unknown) => c(x).type === "thematicBreak",
      );
  ...
  describe("complex wikitext", () => {
    ...
    it("parses paragraph with links and bold", () => {
      ...
      expect(tree.children).toHaveLength(1);
      const para = expectParagraph(tree);
      expect(countType(para, "strong")).toBeGreaterThan(0);
      expect(countType(para, "link")).toBeGreaterThan(0);
    });
    ...
    it("handles real-world AI article opening", () => {
      ...
      expect(tree.children).toHaveLength(1);
      const para = expectParagraph(tree);
      expect(countType(para, "strong")).toBeGreaterThanOrEqual(2);
      expect(countType(para, "link")).toBeGreaterThanOrEqual(5);
    });
  });
  ...
  describe("edge cases", () => {
    ...
    it("handles multiple consecutive links", () => {
      ...
      expect(tree.children).toHaveLength(1);
      const para = expectParagraph(tree);
      expect(countType(para, "link")).toBe(3);
    });
    ...
    it("handles bold with spaces inside", () => {
      ...
      expect(tree.children).toHaveLength(1);
      const para = expectParagraph(tree);
      const strong = c(para.children?.[0] as unknown);
      expect(strong?.type).toBe("strong");
      const sv = strong?.value || c((strong?.children ?? [])[0])?.value;
      expect(sv).toContain("bold text with spaces");
    });
    ...
    it("handles mixed content with newlines", () => {
      ...
      expect(tree.children.some((x: unknown) => c(x).type === "heading")).toBe(
        true,
      );
      expect(
        tree.children.some((x: unknown) => c(x).type === "thematicBreak"),
      ).toBe(true);
    });
  });
...
describe("templates", () => {
  it("removes simple template", () => {
    ...
    expect(c(para.children?.[0] as unknown)?.value).toBe("text ");
    expect(c(para.children?.[1] as unknown)?.value).toBe(" more");
  });
  ...
  it("removes template at start of paragraph", () => {
    ...
    expect(tree.children).toHaveLength(1);
    const para = expectParagraph(tree);
    expect(c(para.children?.[0] as unknown)?.value).toBe(" text here");
  });
...
describe("category links", () => {
  ...
  it("removes multiple category links", () => {
    ...
    expect(c(para.children?.[0] as unknown)?.value).toBe(" ");
  });
  ...
  it("removes category from within text paragraph", () => {
    ...
    const para = expectParagraph(tree);
    expectTextCount(para, 2);
    expect(c(para.children?.[0] as unknown)?.value).toBe("Some text ");
    expect(c(para.children?.[1] as unknown)?.value).toBe(" more text");
  });
  ...
  it("preserves regular wiki links but removes categories", () => {
    ...
    expect(tree.children).toHaveLength(1);
    const para = expectParagraph(tree);
    expect(countType(para, "link")).toBe(1);
    const link = (para.children ?? []).find(
      (x: unknown) => c(x).type === "link",
    );
    expect(c(link)?.url).toBe("page");
  });
