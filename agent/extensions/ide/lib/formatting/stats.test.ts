...
import { formatFileStats, formatRelativeTime } from "./stats";
...
describe("formatRelativeTime", () => {
  describe("given a time less than an hour ago", () => {
    ...
    it("then returns 0m ago for the current time", () => {
      const now = new Date();
      expect(formatRelativeTime(now.toISOString(), now)).toBe("0m ago");
    });
  });
  ...
  describe("given a time between 1 and 23 hours ago", () => {
    it("then returns hours ago", () => {
      ...
      expect(formatRelativeTime(threeHoursAgo.toISOString(), now)).toBe(
        "3h ago",
      );
    });
    ...
    it("then returns 1h ago for one hour ago", () => {
      ...
      expect(formatRelativeTime(oneHourAgo.toISOString(), now)).toBe("1h ago");
    });
  });
  ...
  describe("given a time between 1 and 29 days ago", () => {
    it("then returns days ago", () => {
      ...
      expect(formatRelativeTime(fiveDaysAgo.toISOString(), now)).toBe("5d ago");
    });

    it("then returns 1d ago for one day ago", () => {
      const now = new Date();
      const oneDayAgo = new Date(now.getTime() - 24 * 60 * 60_000);
      expect(formatRelativeTime(oneDayAgo.toISOString(), now)).toBe("1d ago");
    });
  });
  ...
  describe("given a time 30 or more days ago", () => {
    it("then returns the locale date string", () => {
      ...
      const result = formatRelativeTime(twoMonthsAgo.toISOString(), now);
