...
| `scoreThreshold`       | `number`  | `0.55`                                 | Minimum cosine similarity for error-time reminders  |
...
| `promptScoreThreshold` | `number`  | `0.60`                                 | Minimum cosine similarity for prompt-time reminders |
...
indexer.ts        Orchestrates parsing, embedding, and caching at startup
chunker.ts        Markdown parsing constants and utilities (SKILLS_DIR, deriveSkillName)
query-builder.ts  Builds search queries from tool error events
cache.ts          Local cache path helpers and IndexedSection type
scorer.ts         Cosine scoring and deduplication/ranking
reminder.ts       HitChunk type definition, hit formatting, and reminder building
```
