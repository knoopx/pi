...
// =============================================================================
// Agent discovery and configuration (inlined from agents.ts)
// =============================================================================
...
function buildEntryComponent(
  entry: LoopViewerEntry,
  theme: any,
  ui: any,
  cwd: string,
  expanded: boolean,
...
function renderLoopEntries(
  entries: LoopViewerEntry[],
  theme: any,
  tui: any,
  cwd: string,
  expanded: boolean,
...
function buildLoopEntries(loopDetails: RalphLoopDetails): LoopViewerEntry[] {
  ...
  entries.push({
    type: "meta",
    text: `Condition: ${loopDetails.conditionCommand} (${loopDetails.conditionSource})`,
  });
  entries.push({
    type: "meta",
    text: `Iterations: ${loopDetails.iterations.length}`,
  });
  ...
  const appendQueuedEntries = () => {
    ...
    if (loopDetails.steering.length > 0) {
      entries.push({
        type: "note",
        text: `Steering queued: ${loopDetails.steering.join(" | ")}`,
      });
    }
    if (loopDetails.followUps.length > 0) {
      entries.push({
        type: "note",
        text: `Follow-ups queued: ${loopDetails.followUps.join(" | ")}`,
      });
    }
    if (loopDetails.steeringSent.length > 0) {
      entries.push({
        type: "note",
        text: `Steering sent: ${loopDetails.steeringSent.join(" | ")}`,
      });
    }
    if (loopDetails.followUpsSent.length > 0) {
      entries.push({
        type: "note",
        text: `Follow-ups sent: ${loopDetails.followUpsSent.join(" | ")}`,
      });
    }
  };
  ...
  for (const iteration of loopDetails.iterations) {
    entries.push({
      type: "section",
      text: `Iteration ${iteration.index} (${(iteration.details as SubagentDetails).mode})`,
    });

    for (const result of (iteration.details as SubagentDetails).results) {
      ...
      if (result.task)
        entries.push({ type: "note", text: `Task: ${result.task}` });
      if (result.errorMessage)
        entries.push({ type: "note", text: `Error: ${result.errorMessage}` });
        ...
        if (msg.role === "assistant") {
          ...
          toolResults.set(msg.toolCallId, {
            ...
            result: {
              ...
              details: msg.details,
              isError: msg.isError,
              isPartial: (msg as any).isPartial,
            },
          });
        }
      }
...
function cloneLoopParams(params: any): any {
  return {
    ...
    chain: Array.isArray(params.chain)
      ? params.chain.map((step: any) => ({ ...step }))
      : undefined,
  };
}
...
async function runSingleAgent(
  ...
  const subagentSessionDir = path.join(
    os.homedir(),
    ".pi",
    "agent",
    "sessions",
  ...
  const sessionFile = path.join(
    subagentSessionDir,
    `${timestamp}_${safeName}${stepSuffix}${indexSuffix}_${uuid}.jsonl`,
  );
  ...
  if (agent.customTools && agent.customTools.length > 0) {
    const optionalToolsDir = path.join(
      os.homedir(),
      ".pi",
      "agent",
      "tools-optional",
  ...
  try {
    ...
    const exitCode = await new Promise<number>((resolve) => {
      ...
      proc.stdin?.on("error", (error) => {
        markStdinClosed(
          error instanceof Error ? error : new Error("stdin error"),
        );
      });
      proc.stdin?.on("close", () => {
      ...
      const sendFollowUp = (message: string) =>
        sendCommand({ type: "follow_up", message });
      ...
      const upsertToolResult = (
        ...
        const existingIndex = currentResult.messages.findIndex(
          (msg: any) =>
            msg.role === "toolResult" && msg.toolCallId === toolCallId,
        );
        const existing =
          existingIndex >= 0
            ? (currentResult.messages[existingIndex] as any)
            : null;
        const toolMessage = {
          role: "toolResult",
          toolCallId,
        ...
        if (existingIndex >= 0) {
          currentResult.messages[existingIndex] = {
            ...existing,
            ...toolMessage,
          } as Message;
        } else {
      ...
      sendCommand({ type: "prompt", message: `Task: ${task}` })
        .then(() => {
          const followUps = (initialFollowUps ?? []).filter(
...
const ChainItem = Type.Object({
  ...
  model: Type.Optional(
    Type.String({
      description:
        "Override the agent's model (e.g., 'claude-opus-4-5', 'gpt-5.2-codex')",
    }),
  ),
...
const LoopParams = Type.Object({
  ...
  maxIterations: Type.Optional(
    Type.Number({ description: "Max iterations (optional)." }),
  ),
  sleepMs: Type.Optional(
    Type.Number({
      description: `Sleep between iterations in ms (default ${DEFAULT_LOOP_SLEEP_MS}).`,
    }),
  ),
  agent: Type.Optional(
    Type.String({
  ...
  model: Type.Optional(
    Type.String({
      description:
        "Override the agent's model for single mode (e.g., 'claude-opus-4-5', 'gpt-5.2-codex')",
    }),
  ),
  thinking: Type.Optional(ThinkingLevel),
});
...
async function executeSubagentOnce(
  ...
  if (
    ...
    const projectAgentsRequested = Array.from(requestedAgentNames)
      ...
      .filter(
        (a: AgentConfig | undefined): a is AgentConfig =>
          a?.source === "project",
      );
  ...
  if (params.chain && params.chain.length > 0) {
    ...
    for (let i = 0; i < params.chain.length; i++) {
      ...
      const chainUpdate: OnUpdateCallback | undefined = onUpdate
        ? (partial) => {
            const currentResult = (partial.details as SubagentDetails)
              ?.results[0];
            if (currentResult) {
...
async function confirmProjectAgentsOnce(
  params: any,
  ctx: any,
): Promise<boolean> {
  const agentScope: AgentScope = params.agentScope ?? "user";
  ...
  const projectAgentsRequested = Array.from(requestedAgentNames)
    .map((name) => agents.find((a) => a.name === name))
    .filter(
      (a: AgentConfig | undefined): a is AgentConfig => a?.source === "project",
    );
...
function writeLargeOutputToTempFile(
  prefix: string,
  output: string,
): string | null {
  try {
...
function formatTailTruncationNotice(
  truncation: any,
  ...
  const fullOutputLabel = fullOutputPath
    ? `Full output: ${fullOutputPath}`
    : "Full output: (failed to save)";
...
function formatLastOutputForSummary(lastOutput: string): {
  text: string;
  fullOutputPath: string | null;
} {
  ...
  const notice = formatTailTruncationNotice(
    truncation,
    fullOutputPath,
    lastOutput,
  );
...
function inferConditionCommandFromText(
  ...
  for (const line of lines) {
    const match = line.match(
      /^(?:\s*)(?:condition|exit condition|loop condition)\s*:\s*(.+)$/i,
    );
...
function formatLoopPromptItem(
  ...
  const preview =
    task.length > maxTaskLength ? `${task.slice(0, maxTaskLength)}...` : task;
...
export default function (pi: ExtensionAPI) {
    ...
    handler: async (args: any, ctx: ToolContext) => {
      ...
      if (!message) {
        const input = await ctx.ui.input(
          "Steer ralph_loop:",
          "Add guidance for next iterations",
        );
        if (!input) return;
        message = input.trim();
      }
      ...
      if (sentToActive) {
        ...
        ctx.ui.notify(
          once
            ? "One-off steering queued for next iteration."
  ...
  pi.registerCommand("ralph-resume", {
    ...
    handler: async (_args, ctx) => {
      ...
      ctx.ui.notify(
        hadRuns ? "Resumed current iteration." : "Cleared paused state.",
        "info",
      );
      ctx.ui.setStatus("ralph-loop", getLoopStatusLine());
    },
    ...
    handler: async (_args, ctx) => {
      ...
      const maxLabel =
        ...
        maxIterations !== Number.MAX_SAFE_INTEGER
          ...
          : "";
      const steeringCount =
        loopControl.steering.length + loopControl.steeringOnce.length;
      ...
      if (loopControl.status === "idle" && details?.stopReason)
        parts.push(`Last stop: ${details.stopReason}`);
...
Supports both single agent and multi-agent chain execution modes.`,
    ...
    async execute(_toolCallId, params, onUpdate, ctx, signal) {
      ...
      const buildDetails = (
        overrides: Partial<RalphLoopDetails>,
      ): RalphLoopDetails => ({
        iterations: [],
      ...
      if (agents.length === 0) {
        return {
          content: [
            {
              type: "text",
              text: "No agents available for the selected scope.",
            },
          ],
      ...
      if (modeCount > 1) {
        return {
          content: [
            {
              type: "text",
              text: "Provide exactly one mode (single or chain).",
            },
          ],
      ...
      if (hasSingle) {
        ...
        if (!loopParams.task) {
          ...
          if (!inferredTask) {
            return {
              content: [
                { type: "text", text: "Unable to infer task. Provide a task." },
              ],
              details: buildDetails({ stopReason: "missing-task" }),
              isError: true,
            };
      ...
      const conditionCandidate =
        typeof params.conditionCommand === "string" &&
      ...
      if (conditionCandidate) {
        ...
        if (typeof loopParams.task === "string")
          textForInference = loopParams.task;
        else if (Array.isArray(loopParams.chain) && loopParams.chain.length > 0)
          textForInference = loopParams.chain[0].task;
      ...
      if (maxIterations === null || maxIterations <= 0) {
        return {
          content: [
            { type: "text", text: "maxIterations must be a positive number." },
          ],
      ...
      if (sleepMs === null || sleepMs < 0) {
        return {
          content: [
            {
              type: "text",
              text: "sleepMs must be zero or a positive number.",
            },
          ],
          details: buildDetails({
            stopReason: "invalid-params",
      ...
      const confirmProjectAgents = params.confirmProjectAgents ?? true;
      const approved = confirmProjectAgents
        ? await confirmProjectAgentsOnce(loopParams, ctx)
        : true;
      if (!approved) {
        return {
          content: [
      ...
      const buildLoopDetails = (
        currentIterations: LoopIterationResult[],
      ): RalphLoopDetails => {
      ...
      for (let i = 0; i < maxIterations; i++) {
        ...
        const condition = await checkLoopCondition(
          pi,
          conditionCommand,
          ctx.cwd,
          mergedSignal,
          ...
          ? (partial) => {
              ...
              const streamingIterations = [
                ...iterations,
                {
                  ...
                  isError: (partial as any).isError,
                },
              ];
              ...
              onUpdate({
                content: partial.content ?? [
                  {
                    type: "text",
                    text: `ralph-loop iteration ${iterationIndex} running...`,
                  },
                ],
                details,
              });
        ...
        const steeringText = formatSteeringText([
          ...loopControl.steering,
          ...loopControl.steeringOnce,
        ]);
        const iterationParams = applySteeringToParams(
          baseLoopParams,
          steeringText,
        );
        ...
        loopControl.steeringOnce =
          loopControl.steeringOnce.slice(steeringOnceCount);
        loopControl.steeringSent = [];
        loopControl.followUpsSent = [];
      ...
      const lastOutput =
        iterations.length > 0 ? iterations[iterations.length - 1].output : "";
      const summaryLines = [
      ...
      if (lastCondition.stdout)
        summaryLines.push(`Condition stdout: ${lastCondition.stdout}`);
      if (lastCondition.stderr)
        summaryLines.push(`Condition stderr: ${lastCondition.stderr}`);
      if (lastCondition.exitCode !== 0)
        summaryLines.push(`Condition exit code: ${lastCondition.exitCode}`);
      if (errorMessage && errorMessage !== lastOutput)
    ...
    renderCall(args, theme) {
      ...
      const hasChain =
        Array.isArray(args.chain) && (args.chain as any[]).length > 0;
      const hasSingle = Boolean(args.agent || args.task);
      const mode = hasChain
        ? `chain (${(args.chain as any[]).length} steps)`
        : hasSingle
          ? `single ${args.agent || "(auto)"}`
          : "auto";
      const condition = args.conditionCommand
        ? `cond: ${args.conditionCommand}`
        : "cond: (auto)";
      ...
      if (promptInfo.items.length > 0) {
        ...
        const more =
          promptInfo.items.length > 1
            ? ` +${promptInfo.items.length - 1} more`
            : "";
    ...
    renderResult(result, { expanded }, theme) {
      ...
      const mainBox = new Box(1, 0, (text: string) =>
      ...
      const entriesComponent = renderLoopEntries(
        buildLoopEntries(details),
        theme,
        undefined,
        process.cwd(),
        expanded,
