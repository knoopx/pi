 ...
 * LSP Hover Tool
 *
 * Get hover information for the symbol at a specific position
 */
 ...
 * LSP Hover Tool
 */
export function lspHoverTool(api: ExtensionAPI) {
  return api.registerTool({
    name: "lsp-hover",
    label: "Hover",
    description: "Get hover information for the symbol at a specific position",
    ...
    async execute(
      ...
      if (rLine === undefined || rCol === undefined) {
        throw new Error(
          `Action "hover" requires line/column or a query matching a symbol.`,
        );
      }
    ...
    renderCall(_args, theme) {
      ...
      let text =
        theme.fg("toolTitle", theme.bold("lsp-hover ")) +
