 ...
 * LSP Diagnostics Tool
 */
export function lspDiagnosticsTool(api: ExtensionAPI) {
  return api.registerTool({
    ...
    async execute(
      ...
      if (signal?.aborted) return cancelledToolResult();
      if (!result.receivedResponse) return cancelledToolResult();
    ...
    renderCall(_args, theme) {
      ...
      let text =
        theme.fg("toolTitle", theme.bold("lsp-diagnostics ")) +
        theme.fg("accent", "diagnostics");
