...
setupWebsearchTest("NPM Extension", (ctx) => {
    ...
    describe("when registering tools", () => {
      it("then it should register npm search tool", () => {
        expect(ctx.mockPi.registerTool).toHaveBeenCalledWith(
          expect.objectContaining({ name: "npm-search-packages" }),
        );
      });
    });
  });

  describe("npm-search-packages", () => {
    let registeredTool: MockTool;

    beforeEach(() => {
      const found = ctx.mockPi.registerTool.mock.calls.find(
        (call) => (call[0] as MockTool).name === "npm-search-packages",
      );
      if (!found) throw new Error("npm-search-packages tool not registered");
