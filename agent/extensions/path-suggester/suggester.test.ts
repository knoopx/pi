...
describe("scoreAndRank", () => {
  describe("given an empty index", () => {
    it("then it should return an empty array", async () => {
      const { scoreAndRank } = await import("./suggester");
  ...
  describe("given entries with high similarity scores", () => {
    ...
    it("then it should return all entries above the threshold", async () => {
      const { scoreAndRank } = await import("./suggester");
      const index = [
  ...
  describe("given entries with mixed scores", () => {
    beforeEach(() => {
      ...
      cosineMock.mockImplementation((_a, _b) => {
        callCount++;
        return callCount <= 1 ? 0.9 : 0.5;
      });
    });

    it("then it should filter out entries below the threshold", async () => {
      const { scoreAndRank } = await import("./suggester");
      const index = [
  ...
  describe("given a custom score threshold", () => {
    it("then it should respect the higher threshold", async () => {
  ...
  describe("given more results than maxSuggestions", () => {
    ...
    it("then it should return only maxSuggestions entries", async () => {
      const { scoreAndRank } = await import("./suggester");
      const index = [
        makeEntry("/project/src/a.ts", [1, 0]),
        makeEntry("/project/src/b.ts", [1, 0]),
  ...
  describe("given entries with different scores", () => {
    beforeEach(() => {
      ...
      let callCount = 0;
      cosineMock.mockImplementation((_a, _b) => {
        return scores[callCount++] ?? 0;
      });
    });

    it("then it should sort results by score descending", async () => {
      const { scoreAndRank } = await import("./suggester");
      const index = [
  ...
  describe("given absolute paths", () => {
    ...
    it("then relPath should be relative to projectDir", async () => {
      const { scoreAndRank } = await import("./suggester");
      const index = [
  ...
  describe("given the same path for projectDir and entry", () => {
    ...
    it("then relPath should not contain the projectDir prefix", async () => {
      const { scoreAndRank } = await import("./suggester");
