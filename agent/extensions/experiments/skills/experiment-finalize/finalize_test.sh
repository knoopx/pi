...
# Creates temp git repos, simulates experiment sessions, and verifies behavior.
...
# Create a fresh test repo with a simulated experiment session
...
setup_repo() {
	...
	# Experiment branch
	git checkout -b experiment/test-session --quiet

	# Session files (should not end up in branches)
	echo '{"type":"config","metricName":"ms","metricUnit":"ms","bestDirection":"lower"}' >experiment.jsonl
	echo "# Experiment session" >experiment.md
	echo "#!/bin/bash" >experiment.sh
	echo "- try X" >experiment.ideas.md
...
EOF
	...
	local OUTPUT
	OUTPUT=$(bash "$FINALIZE" $REPO/groups.json 2>&1) || {
		fail_test "basic two groups" "Script failed: $OUTPUT"
		cleanup_repo "$REPO"
		return
	}

	# Check branches exist
	git rev-parse "experiment/test/01-optimize-a" >/dev/null 2>&1 || {
		fail_test "basic two groups" "Branch 01 not created"
		cleanup_repo "$REPO"
		return
	}
	git rev-parse "experiment/test/02-optimize-b" >/dev/null 2>&1 || {
		fail_test "basic two groups" "Branch 02 not created"
		cleanup_repo "$REPO"
		return
	}
	...
	BASE_OF_A=$(git merge-base "experiment/test/01-optimize-a" main)
	BASE_OF_B=$(git merge-base "experiment/test/02-optimize-b" main)
	[ "$BASE_OF_A" = "$BASE" ] || {
		fail_test "basic two groups" "Branch A not from merge-base"
		cleanup_repo "$REPO"
		return
	}
	[ "$BASE_OF_B" = "$BASE" ] || {
		fail_test "basic two groups" "Branch B not from merge-base"
		cleanup_repo "$REPO"
		return
	}
	...
	COUNT_A=$(git rev-list --count "$BASE"..experiment/test/01-optimize-a)
	COUNT_B=$(git rev-list --count "$BASE"..experiment/test/02-optimize-b)
	[ "$COUNT_A" = "1" ] || {
		fail_test "basic two groups" "Branch A has $COUNT_A commits, expected 1"
		cleanup_repo "$REPO"
		return
	}
	[ "$COUNT_B" = "1" ] || {
		fail_test "basic two groups" "Branch B has $COUNT_B commits, expected 1"
		cleanup_repo "$REPO"
		return
	}
	...
	A_CONTENT=$(git show experiment/test/01-optimize-a:file_a.txt)
	B_CONTENT=$(git show experiment/test/02-optimize-b:file_b.txt)
	[ "$A_CONTENT" = "optimized_a" ] || {
		fail_test "basic two groups" "file_a.txt wrong in branch A: $A_CONTENT"
		cleanup_repo "$REPO"
		return
	}
	[ "$B_CONTENT" = "optimized_b" ] || {
		fail_test "basic two groups" "file_b.txt wrong in branch B: $B_CONTENT"
		cleanup_repo "$REPO"
		return
	}
	...
	A_FILEB=$(git show experiment/test/01-optimize-a:file_b.txt)
	B_FILEA=$(git show experiment/test/02-optimize-b:file_a.txt)
	[ "$A_FILEB" = "original" ] || {
		fail_test "basic two groups" "Branch A has file_b changes"
		cleanup_repo "$REPO"
		return
	}
	[ "$B_FILEA" = "original" ] || {
		fail_test "basic two groups" "Branch B has file_a changes"
		cleanup_repo "$REPO"
		return
	}
...
EOF
	...
	bash "$FINALIZE" $REPO/groups.json >/dev/null 2>&1 || {
		fail_test "no session artifacts" "Script failed"
		cleanup_repo "$REPO"
		return
	}

	for f in experiment.jsonl experiment.sh experiment.md experiment.ideas.md; do
		if git show "experiment/test/01-all":"$f" &>/dev/null 2>&1; then
...
EOF
	...
	bash "$FINALIZE" $REPO/groups.json >/dev/null 2>&1 && {
		fail_test "rollback on failure" "Script should have failed"
		cleanup_repo "$REPO"
		return
	}
	...
	[ "$CURRENT" = "experiment/test-session" ] || {
		fail_test "rollback on failure" "Not on original branch: $CURRENT"
		cleanup_repo "$REPO"
		return
	}

	# No leftover branches
	if git rev-parse "experiment/test/01-good" >/dev/null 2>&1; then
...
EOF
	...
	local OUTPUT
	OUTPUT=$(bash "$FINALIZE" "$GROUPS_JSON" 2>&1) || {
		fail_test "summary output" "Script failed"
		rm -f "$GROUPS_JSON"
		cleanup_repo "$REPO"
		return
	}
	...
	echo "$OUTPUT" | grep -q "Optimize file A" || {
		fail_test "summary output" "Missing group title in output"
		rm -f "$GROUPS_JSON"
		cleanup_repo "$REPO"
		return
	}
	echo "$OUTPUT" | grep -q "Cleanup" || {
		fail_test "summary output" "Missing cleanup in output"
		rm -f "$GROUPS_JSON"
		cleanup_repo "$REPO"
		return
	}
	echo "$OUTPUT" | grep -q "experiment.ideas.md" || {
		fail_test "summary output" "Missing ideas in output"
		rm -f "$GROUPS_JSON"
		cleanup_repo "$REPO"
		return
	}
	...
	[ ! -f "experiment-finalize-summary.md" ] || {
		fail_test "summary output" "Summary file written to disk — should only print"
		rm -f "$GROUPS_JSON"
		cleanup_repo "$REPO"
		return
	}
...
EOF
	...
	local OUTPUT
	OUTPUT=$(bash "$FINALIZE" $REPO/groups.json 2>&1) || {
		fail_test "stash on dirty tree" "Script failed: $OUTPUT"
		cleanup_repo "$REPO"
		return
	}
	...
	echo "$OUTPUT" | grep -qi "stash" || {
		fail_test "stash on dirty tree" "No stash warning in output"
		cleanup_repo "$REPO"
		return
	}
...
test_no_args() {
	...
	echo "$OUTPUT" | grep -q "Usage:" || {
		fail_test "no args" "No usage message: $OUTPUT"
		return
	}
...
EOF
	...
	echo "$OUTPUT" | grep -qi "detached" || {
		fail_test "detached head" "Wrong error: $OUTPUT"
		cleanup_repo "$REPO"
		return
	}
...
EOF
	...
	echo "$OUTPUT" | grep -qi "trunk" || {
		fail_test "on trunk" "Wrong error: $OUTPUT"
		cleanup_repo "$REPO"
		return
	}
...
test_malformed_json() {
	...
	echo "$OUTPUT" | grep -q "check JSON syntax" || {
		fail_test "malformed json" "Wrong error: $OUTPUT"
		cleanup_repo "$REPO"
		return
	}
...
test_branch_collision() {
	...
	git branch "experiment/test/01-optimize-a" "$BASE"
...
EOF
	...
	echo "$OUTPUT" | grep -q "already exists" || {
		fail_test "branch collision" "Wrong error: $OUTPUT"
		cleanup_repo "$REPO"
		return
	}
...
test_missing_groups_json() {
	...
	echo "$OUTPUT" | grep -q "not found" || {
		fail_test "missing groups.json" "Wrong error: $OUTPUT"
		return
	}
...
EOF
	...
	bash "$FINALIZE" $REPO/groups.json >/dev/null 2>&1 || {
		fail_test "single group" "Script failed"
		cleanup_repo "$REPO"
		return
	}

	git rev-parse "experiment/test/01-all" >/dev/null 2>&1 || {
		fail_test "single group" "Branch not created"
		cleanup_repo "$REPO"
		return
	}
	...
	local A B
	A=$(git show experiment/test/01-all:file_a.txt)
	B=$(git show experiment/test/01-all:file_b.txt)
	[ "$A" = "optimized_a" ] || {
		fail_test "single group" "file_a wrong: $A"
		cleanup_repo "$REPO"
		return
	}
	[ "$B" = "optimized_b" ] || {
		fail_test "single group" "file_b wrong: $B"
		cleanup_repo "$REPO"
		return
	}
...
test_nested_session_artifacts() {
	...
	git checkout -b experiment/nested-test --quiet

	# Session files in a subdirectory (like world's libraries/javascript/polaris/)
	echo '{"type":"config"}' >libs/polaris/experiment.jsonl
	echo "# session" >libs/polaris/experiment.md
	echo "#!/bin/bash" >libs/polaris/experiment.sh
	echo "- idea" >libs/polaris/experiment.ideas.md
	echo "#!/bin/bash" >libs/polaris/experiment.checks.sh
...
EOF
	...
	local OUTPUT
	OUTPUT=$(bash "$FINALIZE" "$REPO/groups.json" 2>&1) || {
		fail_test "nested session artifacts" "Script failed: $OUTPUT"
		cleanup_repo "$REPO"
		return
	}

	# Branch should only have component.ts, not any experiment.* files
	local BRANCH="experiment/test/01-optimize"
	...
	[ "$CONTENT" = "optimized" ] || {
		fail_test "nested session artifacts" "component.ts wrong: $CONTENT"
		cleanup_repo "$REPO"
		return
	}
...
EOF
	...
	git rev-parse "experiment/test/01-optimize-a" >/dev/null 2>&1 || {
		fail_test "verify failure leaves branches" "Branch was deleted"
		cleanup_repo "$REPO"
		return
	}
	...
	CURRENT=$(git branch --show-current 2>/dev/null || echo "")
	[ "$CURRENT" = "experiment/test-session" ] || {
		fail_test "verify failure leaves branches" "Not on original branch: $CURRENT"
		cleanup_repo "$REPO"
		return
	}
...
EOF
	...
	bash "$FINALIZE" "$REPO/groups.json" >/dev/null 2>&1 && {
		fail_test "stash pop on rollback" "Script should have failed"
		cleanup_repo "$REPO"
		return
	}
	...
	CURRENT=$(git branch --show-current 2>/dev/null || echo "")
	[ "$CURRENT" = "experiment/test-session" ] || {
		fail_test "stash pop on rollback" "Not on original branch: $CURRENT"
		cleanup_repo "$REPO"
		return
	}
	...
	[ "$C_CONTENT" = "dirty_tracked" ] || {
		fail_test "stash pop on rollback" "Stash not popped — file_c.txt is '$C_CONTENT'"
		cleanup_repo "$REPO"
		return
	}
...
EOF
	...
	local OUTPUT
	OUTPUT=$(bash "$FINALIZE" "$REPO/groups.json" 2>&1) || {
		fail_test "skipped empty group" "Script failed: $OUTPUT"
		cleanup_repo "$REPO"
		return
	}
	...
	[ "$COUNT" = "1" ] || {
		fail_test "skipped empty group" "No creation summary found"
		cleanup_repo "$REPO"
		return
	}
	echo "$OUTPUT" | grep -q "Created 2 branches" || {
		fail_test "skipped empty group" "Expected 2 branches created"
		cleanup_repo "$REPO"
		return
	}
	...
	echo "$OUTPUT" | grep -q "skipping" || {
		fail_test "skipped empty group" "No skip warning"
		cleanup_repo "$REPO"
		return
	}
...
test_three_groups() {
	...
	git checkout -b experiment/three-test --quiet
...
EOF
	...
	local OUTPUT
	OUTPUT=$(bash "$FINALIZE" "$REPO/groups.json" 2>&1) || {
		fail_test "three groups" "Script failed: $OUTPUT"
		cleanup_repo "$REPO"
		return
	}

	# All 3 branches exist
	git rev-parse "experiment/three/01-a" >/dev/null 2>&1 || {
		fail_test "three groups" "Branch 01 missing"
		cleanup_repo "$REPO"
		return
	}
	git rev-parse "experiment/three/02-b" >/dev/null 2>&1 || {
		fail_test "three groups" "Branch 02 missing"
		cleanup_repo "$REPO"
		return
	}
	git rev-parse "experiment/three/03-c" >/dev/null 2>&1 || {
		fail_test "three groups" "Branch 03 missing"
		cleanup_repo "$REPO"
		return
	}

	# Each branch is independent — 1 commit from base
	for b in experiment/three/01-a experiment/three/02-b experiment/three/03-c; do
		local C
		C=$(git rev-list --count "$BASE".."$b")
		[ "$C" = "1" ] || {
			fail_test "three groups" "$b has $C commits, expected 1"
			cleanup_repo "$REPO"
			return
		}
	done
	...
	[ "$(git show experiment/three/01-a:file_a.txt)" = "optimized_a" ] || {
		fail_test "three groups" "01-a: wrong file_a"
		cleanup_repo "$REPO"
		return
	}
	[ "$(git show experiment/three/01-a:file_b.txt)" = "original_b" ] || {
		fail_test "three groups" "01-a: has file_b changes"
		cleanup_repo "$REPO"
		return
	}
	[ "$(git show experiment/three/02-b:file_b.txt)" = "optimized_b" ] || {
		fail_test "three groups" "02-b: wrong file_b"
		cleanup_repo "$REPO"
		return
	}
	[ "$(git show experiment/three/02-b:file_a.txt)" = "original_a" ] || {
		fail_test "three groups" "02-b: has file_a changes"
		cleanup_repo "$REPO"
		return
	}
	[ "$(git show experiment/three/03-c:file_c.txt)" = "optimized_c" ] || {
		fail_test "three groups" "03-c: wrong file_c"
		cleanup_repo "$REPO"
		return
	}

	# Verify passed (output should contain the checkmark)
	echo "$OUTPUT" | grep -q "Union of all groups matches" || {
		fail_test "three groups" "Verify didn't pass"
		cleanup_repo "$REPO"
		return
	}
