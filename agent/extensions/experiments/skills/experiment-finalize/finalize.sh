...
# experiment-finalize — creates independent branches from an experiment session
#
...
#   "trunk": "main",
#   "final_tree": "<full HEAD hash of experiment branch>",
#   "goal": "short-slug",
#   "groups": [
#     {
...
assert_on_feature_branch() {
  ...
  [ -n "$ORIG_BRANCH" ] || fail "Detached HEAD — switch to the experiment branch first."
  [ "$ORIG_BRANCH" != "$TRUNK" ] || fail "On trunk ($TRUNK) — switch to the experiment branch first."
}
...
preflight() {
  ...
  for i in $(seq 0 $((GROUP_COUNT - 1))); do
    ...
    branch_name="experiment/${GOAL}/${group_number}-$(cat "$DATA_DIR/$i.slug")"
  ...
  assert_branch_available "experiment/${GOAL}/verify-tmp"
...
create_group_branch() {
  ...
  group_number=$(printf "%02d" $((i + 1)))
  branch_name="experiment/${GOAL}/${group_number}-${slug}"
...
verify_union_matches_original() {
  local verify_branch="experiment/${GOAL}/verify-tmp"
  ...
  if [ -n "$non_session_diff" ]; then
    echo -e "${RED}✗ Union of groups differs from experiment branch!${NC}"
    echo "  Files:$non_session_diff"
    return 1
  fi

  echo -e "${GREEN}✓ Union of all groups matches original experiment branch.${NC}"
  return 0
}
...
print_summary() {
  ...
  echo ""
  echo "Cleanup — after merging, delete the experiment branch and session files:"
  echo ""
  echo "  git branch -D $ORIG_BRANCH"
  echo "  rm -f experiment.jsonl experiment.sh experiment.md experiment.ideas.md"

  if [ -f "experiment.ideas.md" ]; then
    echo ""
    echo "Ideas backlog (from experiment.ideas.md):"
    echo ""
    sed 's/^/  /' experiment.ideas.md
  fi
