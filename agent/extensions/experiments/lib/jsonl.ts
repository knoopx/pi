...
export interface ExperimentConfigEntry extends JsonlEntry {
  type: "config";
  name?: string;
  metricName?: string;
  metricUnit?: string;
...
export interface ExperimentRunEntry extends JsonlEntry {
  run: number;
}
...
function runFrom(entry: ExperimentRunEntry, segment: number): ReconstructedRun {
  return {
...
export function isExperimentConfigEntry(
  entry: unknown,
): entry is ExperimentConfigEntry {
  return isObjectRecord(entry) && entry.type === "config";
}

export function isExperimentRunEntry(
  entry: unknown,
): entry is ExperimentRunEntry {
  return isObjectRecord(entry) && typeof entry.run === "number";
}

function firstConfigEntry(jsonlContent: string): ExperimentConfigEntry | null {
  for (const line of nonEmptyLines(jsonlContent)) {
    const entry = parseJsonlEntry(line);
    if (isExperimentConfigEntry(entry)) return entry;
  }
  return null;
}

export function hasExperimentConfigHeader(jsonlContent: string): boolean {
  return firstConfigEntry(jsonlContent) !== null;
}

export function extractExperimentSessionName(jsonlContent: string): string {
  return firstConfigEntry(jsonlContent)?.name || "Experiment";
}
...
export function reconstructJsonlState(
  ...
  for (const line of nonEmptyLines(jsonlContent)) {
    ...
    if (isExperimentConfigEntry(entry)) {
    ...
    if (!isExperimentRunEntry(entry)) continue;
