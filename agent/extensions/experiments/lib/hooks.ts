...
import { hasExperimentConfigHeader } from "./jsonl";

const TIMEOUT_MS = 30_000;
const STDOUT_MAX_BYTES = 8 * 1024;
const TRUNCATION_MARKER = "\n…[truncated: hook stdout exceeded 8KB]";
const HOOKS_DIR = "experiment.hooks";
...
function hasConfigHeader(jsonlPath: string): boolean {
  ...
  try {
    return hasExperimentConfigHeader(fs.readFileSync(jsonlPath, "utf-8"));
  } catch {
    return false;
  }
}
