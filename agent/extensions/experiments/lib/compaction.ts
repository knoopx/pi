// Deterministic compaction summary for experiment sessions.
//
...
export interface ExperimentSummaryPaths {
  workDir: string;
  jsonlPath: string;
  mdPath: string;
  ideasPath: string;
...
export function experimentSummaryPathsFor(
  workDir: string,
): ExperimentSummaryPaths {
  return {
    workDir,
    jsonlPath: path.join(workDir, "experiment.jsonl"),
    mdPath: path.join(workDir, "experiment.md"),
    ideasPath: path.join(workDir, "experiment.ideas.md"),
  };
}

export function buildExperimentCompactionSummary(
  paths: ExperimentSummaryPaths,
): string {
...
function headerSection(): string {
  return [
    ...
    "",
    "The conversation history was discarded; the persisted experiment state below is the source of truth.",
