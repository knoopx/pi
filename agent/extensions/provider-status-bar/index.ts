...
async function fetchCodexUsage(modelRegistry: any): Promise<UsageSnapshot> {
    ...
    try {
        ...
        if (res.status === 401 || res.status === 403) {
            return { provider: "openai-codex", displayName: "Codex", windows: [], error: "Token expired" };
        }

        if (!res.ok) {
            return { provider: "openai-codex", displayName: "Codex", windows: [], error: `HTTP ${res.status}` };
        }
        ...
        return { provider: "openai-codex", displayName: "Codex", windows: [], error: String(e) };
    }
}
...
async function fetchUsageData(
  ...
  return usages.filter((u) => u.windows.length > 0 || u.error).sort((a, b) => a.provider.localeCompare(b.provider));
}
