...
In a single-user installation, `/nix` is owned by the invoking user. The script will invoke `sudo` to create `/nix` if it doesn’t already exist.
...
- Choose a [version](https://releases.nixos.org/?prefix=nix/) and [system type](/manual/nix/2.28/development/building#platforms)
...
We believe we have ironed out how to cleanly support the read-only root file system on modern macOS.

This section previously detailed the situation, options, and trade-offs, but it now only outlines what the installer does.
