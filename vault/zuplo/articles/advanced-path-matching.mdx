...
use URLPattern matching you must set your route's `x-zuplo-path.pathMode` to `url-pattern`.
...
Use [URLPattern.com](https://urlpattern.com) to test your URL Patterns in the browser.

:::

## Trailing Slashes

In URLPattern, trailing slashes aren't accepted unless explicitly specified.
...
URLPattern supports dynamic matching including a feature that will parameterize parts of the URL.
...
You can write a [URL Rewrite](../handlers/url-rewrite.mdx) that takes an incoming wildcard and appends it to the backend request, for example Path: `/foo/bar:path(/.*)` Incoming URL: `/foo/bar/apple/banana` URL Rewrite Pattern: `https://example.com/x${params.path}` Outgoing URL: `https://example.com/x/apple/banana`

## Not supported

not all regex features are available, us of the following will either be ignored or result in an error, including
