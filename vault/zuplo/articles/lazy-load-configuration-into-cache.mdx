...
The next fastest place is in [ZoneCache](../programmable-api/zone-cache.mdx) which is a cache located in each data center.

The [MemoryZoneReadThroughCache](../programmable-api/memory-zone-read-through-cache.mdx) offers the best of both worlds - it uses memory and zone cache in combination to afford the lowest possible latency.

:::warning

Do take care not to load so much data into memory that you OOM (out-of-memory) your process.
