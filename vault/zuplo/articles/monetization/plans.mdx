...
Plan --> Phases Phases --> RateCards RateCards --> Price RateCards --> Entitlement Entitlement --> Feature Feature --> Meter`}
...
Plans contain **phases**, and each phase contains **rate cards** that define pricing and entitlements.

:::note

Before creating a plan, you must first define the [Features](./features.mdx) that will be included.
...
For step-by-step examples building plans from simple to complex, see [Plan Examples](./plan-examples.mdx).
...
For complete API operations (list, get, update, delete, archive), see the [Plans API Reference](../../api/metering-plans).
