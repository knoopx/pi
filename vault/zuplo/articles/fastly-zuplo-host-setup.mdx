...
This document outlines how to use Zuplo as a host for Fastly. While this isn't a necessary setup for most people as Zuplo already runs at the edge and can be used for CDN like caching.
...
It's recommended that you use a custom domain for your Zuplo API Gateway.
...
`api.example.com` if using a custom domain, or `my-project-main-021839d.zuplo.app` without a custom domain.
...
domain. If using a `zuplo.app` domain, set the set the _Certificate Hostname_ to `zuplo.app`.
