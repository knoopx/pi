...
Zuplo's managed edge deployment has a 500MB request body size limit.

:::tip{title="Managed Dedicated"}

If you require larger request sizes you can consider Zuplo's [Managed Dedicated](../dedicated/overview.mdx) offering which allows custom request size limits.
...
If you are developing locally and want code completion, etc in your project, install the AWS SDK for S3 to your project.
...
Create a new module in your Zuplo project that generates pre-signed URLs.
...
Pre-signed URLs expire after the specified duration (default: 1 hour in the example).
...
While S3 can handle files up to 5TB, you may want to enforce size limits.
