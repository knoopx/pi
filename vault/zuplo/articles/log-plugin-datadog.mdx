...
To add the Datadog logging plugin to your Zuplo project, add the following code to your `zuplo.runtime.ts` file.
...
The `DataDogLoggingPlugin` constructor accepts an options object with the following properties:
...
[tags](https://docs.datadoghq.com/getting_started/tagging/) to include in each log entry as key-value pairs
...
Any custom tags you want to include in the log entry can be added to the `tags` property.
