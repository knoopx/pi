...
To add the VMware Log Insight logging plugin to your Zuplo project, add the following code to your `zuplo.runtime.ts` file.
...
The `VMwareLogInsightLoggingPlugin` constructor accepts an options object with the following properties:
...
Any custom fields you want to include in the log entry can be added to the `fields` property.

### Text Replacements

The `textReplacements` option allows you to specify character replacements in the log text.

## Default Fields

Every log entry will have a `timestamp` and a `text` field. The text field contains the log message, which may be JSON encoded for complex data.
