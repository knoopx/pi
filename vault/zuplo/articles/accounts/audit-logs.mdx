...
Audit logs provide a comprehensive record of activities within your Zuplo account, helping you track changes and maintain compliance requirements.

:::note{title="Beta Feature"}

Audit logs are currently in beta. During the beta period not all events may be captured.
...
Date Range Limitation The date range between `startDate` and `endDate` can't exceed 30 days.
