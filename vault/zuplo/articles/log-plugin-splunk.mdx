...
To add the Splunk logging plugin to your Zuplo project, add the following code to your `zuplo.runtime.ts` file.
...
The `SplunkLoggingPlugin` constructor accepts an options object with the following properties:
...
acknowledgment. If not provided, the X-Splunk-Request-Channel header won't be sent
