...
When you create a new project you will see a `routes.oas.json` file. This is your default OpenAPI routing file.
...
This will merge your imported file with any routes you already have in your .oas.json file.
...
The import feature will merge changes from an external source-of-truth OpenAPI document and will keep your Zuplo settings intact, while overwriting everything else from your imported OpenAPI docs.
...
`x-zuplo-path` is used to specify the type of [path matching mode](./routing.mdx) you want to use; **open-api** is the default and uses the standard OpenAPI slugs for tokens (for example `/pizza/{size}`).
...
`x-zuplo-route` is where all of your route settings are configured, including [policies](/docs/policies), [handlers](/docs/handlers/openapi.mdx), [CORS](../programmable-api/custom-cors-policy.mdx) and more.
...
You can set this property to `true` in order to hide a route from the Developer Portal.

## Multiple File Support

You can have multiple `.oas.json` files if you wish to break up your route definitions.
