...
Mutual TLS (mTLS) authentication establishes a trust relationship between your Zuplo API Gateway and your backend services using client certificates.
...
Use the Zuplo CLI to upload your client certificate and private key to your project.
...
The certificate name must follow JavaScript's variable naming constraints since you will use the name later in your code.
...
For better flexibility across environments, store the certificate name as an [environment variable](./environment-variables.mdx):
...
You can't delete a certificate that's referenced by any of your deployments in your project.

First, disable the certificate by using the CLI with `zuplo mtls-certificate disable`.
...
mTLS bindings aren't currently available in local development environments.
...
If you need assistance configuring mTLS for your project, contact us at [support@zuplo.com](mailto:support@zuplo.com).
