...
Part of running a reliable API Gateway is ensuring that the service is healthy and available.

A typical health check endpoint on your API Gateway will exercise some of the most important policies (such as authentication) and then make a simple request to your backend to ensure that it's reachable and functioning.

## Setting up a Health Check Handler

To set up a health check handler, you will need to create a new route in your Zuplo Gateway.

You'll create a new route with the path `/health` and the method `GET`.
...
Next, you'll create a custom handler module that will be used to process the health check requests.
