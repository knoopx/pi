...
Refer to Zuplo's documentation on [how to configure Zuplo as a Fastly host](./fastly-zuplo-host-setup.mdx).
...
The information below is a summary of Fastly's own recommendations for securing your backend - regardless of whether you are using Zuplo, another API Gateway, or Fastly origins.

### IP Address Restrictions

Fastly maintains a list of IP addresses that you can use to restrict access to your API Gateway.

In Zuplo, you can utilize the IP Address Restriction policy to limit traffic to only the Fastly IP addresses.
...
Another way to ensure that traffic is coming from Fastly is to use signed headers.

In Fastly, you will need to create a VCL snippet that adds a signed header as shown below.
...
Another way to ensure that traffic is coming from Fastly is to use add a JWT header to the outgoing request.
...
In Fastly, you will need to create a VCL snippet that adds a JWT header as shown below.
...
Fastly supports mTLS authentication for backend services. This is a good way to ensure that only Fastly can access your API Gateway.
