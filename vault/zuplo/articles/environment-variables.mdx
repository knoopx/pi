...
Environment variables are key-value pairs that are stored outside of source code.

Environment variables can be read into source code and many configuration files in your project.

Environment variables can be configuration or secrets. While all values are stored encrypted at rest, only non-secret values can be read.

:::tip

API Reference For detailed information about accessing environment variables in code, see the [Environment Variables API Reference](../programmable-api/environment.mdx).
...
Environment variables can be applied to one or many different environments.
...
For the **Preview** environment option, a specific named environment can be selected.

For the **Working Copy** option, developers can set a personal override.

:::info

A single environment variable name can't overlap environments.
...
unique name for the environment. This is the same name that's used in the URL of the environment.
...
[compatibility date](../programmable-api/compatibility-dates.mdx) of runtime environment.
