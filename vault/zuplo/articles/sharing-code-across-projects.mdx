...
When you have multiple Zuplo projects that share common functionality like custom policies, handlers, or utility functions, you can create a shared npm package to avoid duplicating code.
...
Since Zuplo compiles TypeScript at deployment time, you ship raw TypeScript source files rather than pre-compiled JavaScript.
...
Create a script in your shared package that copies files to the consumer's `modules` folder.
...
The package's `postinstall` script automatically copies the TypeScript files to your `modules/shared` folder.
...
The header comments added by the copy script help identify these files as generated code that should not be edited directly.

### Updating Shared Modules

When you update the shared package version, the `postinstall` script overwrites the existing files with the new version.
