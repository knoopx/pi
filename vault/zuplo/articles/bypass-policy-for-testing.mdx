...
There are times when you need to bypass a policy for purposes such as testing, debugging, writing health checks, etc.
...
The [Custom Code Inbound Policy](../policies/custom-code-inbound.mdx#writing-a-policy) is essentially a wrapper around whatever policy you want to bypass.

This policy first checks for the presence of a `testApiKey` flag in the user's `data` (which is the API Key metadata).
...
To create a test API Key, navigate to **Services** in your Zuplo Project.
...
The claims for a JWT token are added to the `request.user.data` object when you use one of the Zuplo JWT Authentication policies.
