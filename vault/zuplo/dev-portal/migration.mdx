...
This guide walks you through migrating your existing documentation from the sequentially for a smooth transition.

## Before you begin

It's important to note that this migration is to a completely new developer portal.

A few things to keep in mind before starting the migration:

### Separate domain for Dev Portal

New Dev Portals run on their own dedicated domain instead of a `/docs` path under your API.

:::note

Builder plans have been automatically upgraded to include two custom domains instead of one.

:::

### Authentication changes

Not all authentication providers from the old Dev Portal are supported in the new Developer Portal.

### Other considerations

- **Theming**: Custom CSS from the old Dev Portal isn't directly supported. You
will need to reapply any custom styles using the new theming options in Zudoku.
...
wasn't available in the old portal. You may want to use a different logo for dark mode.

### Test in a preview environment

Don't perform the migration directly on your production environment. Instead, create a preview environment to test the migration.
...
If you have a simple Dev Portal setup and prefer to migrate directly in the Zuplo Portal, you can use the built-in migration tool by opening the `config/dev-portal.json` file in the Zuplo Portal and clicking the **Migrate to Zudoku** button at the top of the editor.
...
See the [Source Commands](../cli/source-migrate.mdx) documentation for more details and options.
...
Here's how several fields map from old to new format. See the [configuration](./zudoku/configuration/overview.mdx) documentation for a complete list of options.
...
Move your [sidebar configuration](./zudoku/configuration/navigation.mdx) from `sidebar.json` to the `navigation` array in `zudoku.config.ts`:
...
Create a `docs/public` directory for your images and other static assets.
...
It is critical that you delete the `config/dev-portal.json` file after completing the migration.

:::

1. **Deploy and Verify**

Deploy your changes by either pushing to a git branch or by running [`npx zuplo deploy`](../cli/deploy.mdx).
...
For instructions on theming the dev portal, see [Colors & Theme](./zudoku/customization/colors-theme.mdx) and [Fonts](./zudoku/customization/fonts.mdx).

## Redirect legacy URLs

The previous Dev Portal was hosted on a path on the same domains your Zuplo API (i.e.

If you were using the previous Dev Portal, you can redirect all requests from the legacy path to the new domain using the [Legacy Dev Portal Handler](/docs/handlers/legacy-dev-portal-handler).
...
and redirects to the new Dev Portal domain. You must set the path to the path used by the previous Dev Portal, such as `/docs(.*)`.
...
For more detailed information about the handler, including how to configure proxy mode to keep your developer portal on the same domain, see the [Legacy Dev Portal Handler](/docs/handlers/legacy-dev-portal-handler) documentation.

### Additional redirects

Your new developer portal may also change other paths. To create redirects for specific docs or other path in your new Dev Portal, we recommend using the `redirects` configuration in the `zudoku.config.ts` file.
