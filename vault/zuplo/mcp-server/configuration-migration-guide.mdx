...
This guide explains how to migrate from the deprecated MCP server configuration syntax to the new consolidated format introduced in November 2025.
...
All operations (tools, prompts, resources) now go into a single `operations` array.
...
The new syntax supports an `enabled` flag on all types to conditionally include operations:
...
When `enabled: false`, the operation is skipped during registration without needing to remove it from the configuration.

### Improved Validation

The new format includes OpenAPI schema validation via `oneOf` constraints, providing better IDE support and error messages.
...
The old syntax is currently marked as deprecated but still functional.
