...
The MCP Server Handler supports custom tools that allow you to create sophisticated MCP ([Model Context Protocol](https://modelcontextprotocol.io/introduction)) tools using OpenAPI specifications and custom handler functions.

:::tip

Custom tools give you full programmatic control over tool behavior within an [MCP Server Handler](../handlers/mcp-server.mdx).
...
This utilizes `context.invokeRoute` to invoke various routes on a gateway.

:::note

`context.invokeRoute` _will_ utilize the full inbound and outbound policy pipeline but does not go back out to HTTP: requests stay within the gateway.
...
Then, add this operation as to an MCP server to make it available as a tool:
