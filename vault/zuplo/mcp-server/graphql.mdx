...
The MCP Server Handler supports GraphQL endpoints through the `x-zuplo-route` OpenAPI extension.
...
Add a route that forwards to your GraphQL endpoint and include the `x-zuplo-route.mcp` configuration:
...
Include the GraphQL route in your MCP Server configuration using the `operations` array:
...
The `x-zuplo-route.mcp` configuration for GraphQL supports the following options:
...
For more complex scenarios like bounded mutations or queries with complex logic, you can create custom GraphQL tools as endpoints using the Zuplo [custom MCP tool patterns](./mcp-server-custom-tools.mdx) and the `graphql` library.
...
For more complex custom tools with validation, error handling, and multi-step workflows, see the [Custom Tools documentation](./mcp-server-custom-tools.mdx).
