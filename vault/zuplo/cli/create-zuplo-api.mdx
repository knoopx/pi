...
The `create-zuplo-api` CLI makes it easy to create a new Zuplo API using the default template or an [example](https://github.com/zuplo/zuplo/tree/main/examples) from a public GitHub repository.
...
with. You can use an example name from the official Zuplo repository or a public GitHub URL.
- `--example-path <path-to-example>` - In a rare case, your GitHub URL might
contain a branch name with a slash (for example, bug/fix-1) and the path to the example (for example, foo/bar).
...
You can find the list of available examples in the [Zuplo examples repository](https://github.com/zuplo/zuplo/tree/main/examples).
