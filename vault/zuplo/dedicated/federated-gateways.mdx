...
With a managed dedicated Zuplo instance you can create a federated gateway that allows you to connect multiple Zuplo projects together.
...
you may want to separate different product areas into their own Zuplo projects.
...
different Zuplo projects, you can reduce the risk of causing inadvertent changes to other parts of the API.

## How to Create a Federated Gateway

Managed dedicated Zuplo environments can call other Zuplo environments deployed in the same instance.

You can use most Zuplo request handlers to make these requests (the Lambda handler isn't supported).

For example, to create a federated gateway that forwards requests to another Zuplo environment, you can use the URL Forward handler.
