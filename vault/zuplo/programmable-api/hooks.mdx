...
All hooks can be either synchronous or asynchronous. To make your hook asynchronous, add the `async` keyword to the function.

:::

## Available Hooks

Zuplo provides several hooks for different stages of the request/response pipeline:
...
The `addResponseSendingHook` method adds a hook that fires before the response is sent to the client.
...
The `addResponseSendingFinalHook` method adds a hook that fires immediately before the response is sent to the client.
...
This example demonstrates how all hooks work together in a request/response lifecycle:
