...
There are two built-in CORS policies called `none` and `anything-goes`.
...
Make sure to not have a trailing `/` on your `allowedOrigins`. for example `https://example.com` is valid, `https://example.com/` won't work.
...
You can also use wildcards to match subdomains, for example `*.example.com` will match any subdomain of example.com.
