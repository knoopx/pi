...
Zuplo encourages developers to build APIs with standard and actionable error messages.
...
This will produce and error response in the standard format. Notice that trace information is included automatically.
...
Every status code has a corresponding method in the `HttpProblems` class, so you can use any HTTP status code as needed.
...
Each method on the `HttpProblems` object supports overriding the default values of the problem response with custom values.
...
The most important thing to remember about problem details is that every instance of a particular error should return the same value for `title`.

:::

An example of how to correctly use the `title` and `detail` properties can be demonstrated with an error that tells the user they provided an unexpected value for a query parameter called `take` that implements pagination.
...
It can sometimes be helpful to specify additional properties on the problem response.
...
At times it can be useful to send custom headers with the error response.
...
If you want to set headers without any overrides pass `undefined` to the third argument.
