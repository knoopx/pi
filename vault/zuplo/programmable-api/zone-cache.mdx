...
The ZoneCache stores data in a shared cache. This cache is in the same zone (the same data center or cluster) as your API - it's not globally distributed, but rather designed to be low-latency.

The ZoneCache can store any JSON serializable data. Each cached item has a time-to-live (TTL) after which it expires and is removed from the cache.

There's an demonstration of ZoneCache use in the [Per User Rate Limits Using a Database](../articles/per-user-rate-limits-using-db.mdx) example.
...
Objects that don't serialize cleanly to JSON (like the `Headers` object) won't be readable after storage.
...
When writing to the cache, you may not want to `await` the operation to complete.
...
Use ZoneCache when you need low-latency access to frequently used data that's stored in a single zone.

Avoid using ZoneCache for large datasets or data that needs to be globally distributed, as it's designed for small, simple objects within a single zone.

## Limits

For full Zuplo platform limits see the [Zuplo Limits documentation](../articles/limits.mdx).
