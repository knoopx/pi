...
Each deployment of Zuplo is issued a unique OAuth client identity. This identity can be used to create ID Tokens that can be used to securely identify requests from your Zuplo API that are made to outside services.

To create a Zuplo Identity Token run the following code from withing a policy, handler, or module in your Zuplo API.
...
The Zuplo ID Token can be used as a means of securing your backend API so that only Zuplo can call the API.

## Verifying the Token Using a Library

To verify the JWT token on your own service, you can use any standard JWT library.

Below is an example of how to verify the token using the [`jose`](https://www.npmjs.com/package/jose) JavaScript library.
