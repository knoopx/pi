...
The `CacheStorage` is exposed as the `caches` global object. This object allows you to open instances of a `Cache`.
...
At this time, the `options` parameter will be ignored entirely when running on in a developer environment (for example working copy).
...
If you want to store the value, create a new simple Response and set the `Cache-Control` header.
...
When adding to the cache, headers are used to control how long resources are stored.
