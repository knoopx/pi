...
The `environment` object provides access to environment variables in your Zuplo API gateway.

:::info{title="Configuration Guide"}

For information on how to set and manage environment variables in the Zuplo Portal, see [Configuring Environment Variables](../articles/environment-variables.mdx).
...
Zuplo doesn't provide a built-in `NODE_ENV` variable since it runs on a custom V8 runtime, not Node.js.
