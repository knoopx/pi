...
description:
  Learn how to create a Zuplo policy that routes requests to different backend
  URLs based on the user's country.
tags:
  - custom-code
  - backends
  - deployment
...
When running a global API, you may want to route requests to region-specific backends for better performance, compliance, or data residency requirements.

## How It Works

Zuplo provides geolocation information for every request through the `context.incomingRequestProperties` object.
...
Zuplo provides more than country information. You can use other properties for more granular routing:
...
The geolocation data is determined at the edge, so there's no additional latency for IP lookup.
