...
Zuplo's AI Gateway acts as an intelligent proxy layer that sits between your engineering team's applications and LLM providers like OpenAI, Google Gemini, and others.

## Key Benefits

**Provider Independence**: Switch between LLM providers (OpenAI, Google Gemini, etc.) dynamically without modifying application code.

**Cost Control**: Set spending limits at organization, team, and application levels with hierarchical budgets that cascade down through your structure.
...
**Self-Service Access**: Developers can create applications and access LLMs without needing direct access to provider API keys.
...
Your applications send requests to the Zuplo AI Gateway URL using your Zuplo API key.

## Core Features

### Multi-Provider Support

Configure multiple LLM providers within a single gateway project. Supported providers include OpenAI (GPT-4, GPT-4.5, and other models) and Google Gemini (all model variants).
...
- **Sub-Teams**: Team-specific limits that can't exceed parent limits (for example, $500/day for the Credit Team)
