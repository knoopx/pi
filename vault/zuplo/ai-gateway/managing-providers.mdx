...
Zuplo's AI Gateway supports integration with various [AI providers](./providers.mdx), allowing you to leverage different models and services for your AI applications.
...
[supported providers](./providers.mdx), or [add a custom provider](./custom-providers.mdx).
...
create an API key for each provider see the [provider documentation](./providers.mdx).
...
You can modify the label, API key, and selected models for the provider.

:::caution

Be careful removing models from an provider that's in use as it may break existing applications that rely on the provider.
