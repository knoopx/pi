...
async function main() {
  ...
  for (const sessionFile of sessionFiles) {
    ...
    const messageLines = await buildMessageLines(session.id);
...
async function buildMessageLines(sessionId: string): Promise<PiMessageLine[]> {
  ...
  for (const message of sortedMessages) {
    ...
    // Add assistant-specific fields for assistant messages
    if (message.role === "assistant") {
      ...
      line.message.api = message.providerID ?? "opencode-import";
      line.message.provider = message.providerID ?? "opencode";
      line.message.model = message.modelID ?? "unknown";
      line.message.usage = {
        input: inputTokens,
        output: outputTokens,
        cacheRead: cacheRead,
      ...
      line.message.stopReason = "endTurn";
    }
  ...
  // First entry must have parentId: null - session header is not part of entry tree
  if (lines[0]) {
    lines[0].parentId = null;
  }
...
async function readAllJson<T>(files: string[]): Promise<T[]> {
  ...
  for (const file of files) {
    try {
      ...
      console.warn(
        `Skipping invalid JSON file ${file}: ${toErrorMessage(error)}`,
      );
    }
  }
  return items;
